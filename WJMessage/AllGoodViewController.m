...
@interface AllGoodViewController ()

@end

@implementation AllGoodViewController
...
//        NSDictionary *result = [RequestData SingleGoodPriceListWithUrl:serviceUrl AndUserName:userName AndGoodName:@"all" AndPage:page];
        NSDictionary *result = [RequestData allGoodPriceListWithUrl:serviceUrl AndUserName:userName AndGoodName:@"all" AndPage:page];
        NSLog(@"result :%@",result);
        NSMutableArray *goodPrices = [[NSMutableArray alloc] init];
        for (id dt in result) {
            GoodPrice *good = [[GoodPrice alloc] init];
            NSString *goodName = [dt objectForKey:@"Name"];
            NSString *goodPrice = [dt objectForKey:@"Price"];
            NSString *updateTime = [dt objectForKey:@"UpdateTime"];
            NSString *gId = [dt objectForKey:@"Id"];
...
//        NSDictionary *result = [RequestData SingleGoodPriceListWithUrl:serviceUrl AndUserName:userName AndGoodName:self.keyWord AndPage:searchPage];
        NSDictionary *result = [RequestData allGoodPriceListWithUrl:serviceUrl AndUserName:userName AndGoodName:self.keyWord AndPage:searchPage];
        //NSLog(@"result :%@",result);
        NSMutableArray *goodPrices = [[NSMutableArray alloc] init];
        for (id dt in result) {
            GoodPrice *good = [[GoodPrice alloc] init];
...
{
    static NSString *PriceListTableIdentifier = @"AllGoodTableIdentifier";
...
//    }
    labelTimeValue.text = [[NSString stringWithFormat:@"%@ - ",self.enterpriseName] stringByAppendingString:good.updateTime];
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    [self performSegueWithIdentifier:@"allgoodtogood" sender:self];
}

- (void)prepareForSegue:(UIStoryboardSegue *)segue sender:(id)sender {
    
    if ([segue.identifier isEqualToString:@"allgoodtogood"]) {
...
//    NSDictionary *result = [RequestData SingleGoodPriceListWithUrl:serviceUrl AndUserName:userName AndGoodName:searchTerm AndPage:searchPage];
    NSDictionary *result = [RequestData allGoodPriceListWithUrl:serviceUrl AndUserName:userName AndGoodName:searchTerm AndPage:searchPage];
    //NSLog(@"result :%@",result);
    NSMutableArray *goodPrices = [[NSMutableArray alloc] init];
    for (id dt in result) {
        GoodPrice *good = [[GoodPrice alloc] init];
...
//    NSDictionary *result = [RequestData SingleGoodPriceListWithUrl:serviceUrl AndUserName:userName AndGoodName:@"all" AndPage:1];
    NSDictionary *result = [RequestData allGoodPriceListWithUrl:serviceUrl AndUserName:userName AndGoodName:@"all" AndPage:1];
    NSLog(@"result :%@",result);
    NSMutableArray *goodPrices = [[NSMutableArray alloc] init];
    for (id dt in result) {
        GoodPrice *good = [[GoodPrice alloc] init];
